...
var debug,          // is debug
    devHotServer,      // is hrm mode
    minimize;       // is minimize


if (currentTarget == "build") { // online mode （线上模式）,不需要调试，测试，启动服务，但需要压缩

    debug = false, devHotServer = false, minimize = true;

} else if (currentTarget == "dev") { // dev mode （开发模式）

    debug = true, devHotServer = false, minimize = false;

} else if (currentTarget == "dev-hrm") { // dev HRM mode （热更新模式）

    debug = true, devHotServer = true, minimize = false;
}
...
var PATHS = {
     ...
     * （发布目录）
     * */
    publicPath: devHotServer ? '/webpack-demo-1.0/dist/' : './',
...
var output = {
     ...
     * （文件名称）
     * */
    filename: devHotServer ? 'js/[name].js' : 'js/[name]-[chunkhash:8].js',

    /*
     * The filename of non-entry chunks as relative path inside the output.path directory.
     * （按需加载模块时输出的文件名称）
     * */
    chunkFilename: devHotServer ? 'js/[name].js' : 'js/[name]-[chunkhash:8].js'
}
...
var plugins = [
    ...
    new webpack.DefinePlugin({
         ...
         * （代理的标识）
         * */
        __DEVAPI__: devHotServer ? "/devApi/" : "''",
    }),
     ...
     * （提取css文件到单独的文件中）
     */
    new ExtractTextPlugin(devHotServer ? "css/[name].css" : "css/[name]-[chunkhash:8].css"),
